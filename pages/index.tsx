...
export const getStaticProps: GetStaticProps = async () => {
  const projects = PROJECTS.filter((el: IFrontMatterV2) => el.published).sort(
    (a: IFrontMatterV2, b: IFrontMatterV2) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()
  )
...
const Home: NextPage<IHomeProps> = ({ projects }) => {
  return (
    <Layout metadata={{ title: 'Jilver Pacheco', description: 'Full-stack developer + Electronic engineer' }}>
      {/* <HeroSection /> */}
      <DescriptionSection />
