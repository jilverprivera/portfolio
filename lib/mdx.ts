...
export const getAllFilesMetadata = async (type: string) => {
  ...
  const filesToReturn = files.reduce((accFiles: any, slug) => {
    ...
    const { data, content } = matter(mdxSource)
    return [{ ...data, slug: slug.replace('.mdx', ''), readingTime: readingTime(content) }, ...accFiles]
  }, [])
  return filesToReturn.sort((a: IFrontMatter, b: IFrontMatter) => Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt)))
}
